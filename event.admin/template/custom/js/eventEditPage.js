...
$(document).ready(function() {
  ...
  if(parseFloat($('#max_giftbox').val()) > 0){
  ...
  var total = 0;
  var num_ncoin = parseFloat($("input.input-total-percent-ncoin").val()) || 0;
  var num_card = parseFloat($("input.input-total-percent-card").val()) || 0;
  var num_special = parseFloat($("input.input-total-percent-special").val()) || 0;

  total = num_ncoin + num_card + num_special;
  ...
  if ($('.no-check-ncoin').val()) {
    $( "#check_ncoin" ).trigger( "click" );
  ...
  if ($('.no-check-card').val()) {
    $( "#check_card" ).trigger( "click" );
  ...
  if ($('.no-check-special').val()) {
    $( "#check_special" ).trigger( "click" );
  ...
  $(".btn-edit-event").click(function() {
    ...
    if(check_ncoin_or_card == 2) {
      alert("Please select Card or Ncoin");
      return false;
    }
    ...
    swal({
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          } else if (response.result == "Sorry! You can't change the status.") {
